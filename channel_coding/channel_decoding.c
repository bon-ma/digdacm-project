...
int main() {
    ...
    char binary_output[FRAME_SIZE * 4];  // Buffer to store serialized binary output
    ...
    while (fread(hex_frame, 1, FRAME_SIZE, stdin) == FRAME_SIZE) {
        ...
        // Extract the received FCS (last byte of the frame)
        uint8_t received_crc = frame_bytes[FRAME_SIZE / 2 - 1];

        // Compute the CRC for the frame excluding the FCS
        uint8_t computed_crc = calculate_crc8(frame_bytes, FRAME_SIZE / 2 - 1);

        // Compare the computed CRC with the received CRC
        if (computed_crc != received_crc) {
            // CRC mismatch: output error message to stderr
            fprintf(stderr, "Error: CRC mismatch. Frame corrupted.\n");
            continue;
        }

        // Serialize the frame to binary (excluding the FCS)
